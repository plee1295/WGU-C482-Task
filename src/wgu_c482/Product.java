...
public class Product {
    ...
    public static String validateProduct(String name, int min, int max, int inv, double price, ObservableList<Part> parts, String message) {
        ...
        if (min < 0) {
            message += ("The inventory must be greater than 0.");
        }
        if (price < 0) {
            message += ("The price must be greater than $0");
        }
        if (min > max) {
            message += ("The inventory MIN must be less than the MAX.");
        }
        if (inv < min || inv > max) {
            message += ("Part inventory must be between MIN and MAX values.");
        }
        if (parts.size() < 1) {
            message += ("Product must contain at least 1 part.");
        }
        if (sumParts > price) {
            message += ("Product price must be greater than cost of parts.");
        }
        return message;
    }
}
